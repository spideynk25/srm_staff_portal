...
class SendNotificationToStaffPage extends ConsumerStatefulWidget {
  const SendNotificationToStaffPage({super.key});

  @override
  ConsumerState<SendNotificationToStaffPage> createState() => _SendNotificationToStaffPageState();
}

class _SendNotificationToStaffPageState extends ConsumerState<SendNotificationToStaffPage> {
  List<dynamic>? staffSubjectsData;
  bool isLoading = true; // Default to true to show the loader initially
  ...
  Future<void> _getEmployeeList() async {
    ...
    try {
      ...
      setState(() {
        isLoading = false; // Stop the loader even if there's an error
      });
